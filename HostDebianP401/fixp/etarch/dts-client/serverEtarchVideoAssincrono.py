...
def atualizaListaStreaming(pktEnvioP, dataAtualP, stringPacoteIdP) :
  ...
  file = open("EnvioPacotesEtarchAssincronoServerVideo01.csv","a")
...
def handle_pkt(pkt):
	...
	#requestNumber = retornaNumeroRequisicao(pkt[Raw].load, 'numero')[0]
        ...
        #print('***Requisicao numero ',reques  print("Envio: Pacote " + str(id), flush=True)  
        #pkt.show2()    	
...
def main():
  ...
  #filtro acima estava dando problema, tambem estava pegando icmp
  filtro = 'ether proto ' + str(ARQUITETURA_IPV4) + ' and port ' + str(VIDEO_PORT) + ' and host ' + IP_DST + ' or ' + IP_SRC + \
