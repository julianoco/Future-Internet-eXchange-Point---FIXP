...
class SBBNEConnector(object):
	...
	def sendForwardingRules(self,    , entityId):
...
def roteamentoRedeFisica(entityP, redeFisica) :
...
class SBBWorkspaceManager(object):	
	...
	def workspaceCreate(self):
		#try :
		  ...
		  if(-1 != registeredWorkspace[0]) :
			...
			workspaceRegisterList.append((int(len(workspaceRegisterList)+1), 
						      self.reqWorAtt.workspace_title, 
						      hashlib.sha256(self.reqWorAtt.workspace_title).digest()[:6],
						      int(len(workspaceRegisterList)+1) ) )
	...
	def workspaceAttach(self):
		#try :
		  ...
		  print("Dictionary de attachs: %s" %workspaceEntityAttachList)	
