...
#CONTROLADOR DE SIMULACAO NOVA GENESIS
...
ARQUITETURA_ETARCH_H = 0x1235
ARQUITETURA_ETARCH_H_S = "\x12\x35"
ARQUITETURA_ETARCH_H_U = u"\x12\x35"
ARQUITETURA_FIXP_H   = 0x0900
ARQUITETURA_FIXP_H_S   = "\x09\x00"
ARQUITETURA_ETARCH_S = "1235"
ARQUITETURA_ETARCH_D = 4661
...
class SBBNEConnector(object):
	...
	def sendForwardingRules(self, arquitectureP, workspaceIdP):
		...
		for forwardingRulesSearch in buscamMultipleTuplaInList(forwardingRulesByWorkspace, 
			...
			if forwardingRulesSearch[0] == -1 :
				...
				print("Erro! workspace %d nao configurado na estrutura no vetor de encaminhamentos" %arquitectureIdP)
				exit(1)
				...
				print("Vetor do worspace retornado: ", registeredWorkspace)
				...
				sendData.append([arquitectureP,   #inteiro decimal
						...
						"FIXP_Switch_Ingress.etarch_forward_ng", #nome completo da tabela (string)
						...
						["FIXP_Switch_Ingress.etarch_SetSpec_Group_NG"], #lista de nomes completos da acoes (string)
						...
						statusDrop]) #identificador de drop 0. nao dropa nada, 1. dropa   ..dropar utilizando a chave inteiro decimal) #modificado
			...
			for msg_buffer in buffer_splitter(rawData):
				...
				if cont == 0 :
					...
					if reqObj.type == dts_pb2.ControlRequest.ETCP_ENTITY_REGISTER :
						...
						#envia packet_out	
						(switch, porta) = topografiaRedeFisica(reqEntReg.title, REDE_FISICA_ATUAL)
						...
						if status == dts_pb2.ControlResponse.SUCCESS :
							...
							while ((True) and (count < 3)):
								...
								(sendRet, rawData) = self.sendForwardingRules(ARQUITETURA_ETARCH_D, workspaceId)
						...
						(switch, porta) = topografiaRedeFisica(reqWorAttCre.entity_title, REDE_FISICA_ATUAL)	
						...
						(switch, porta) = topografiaRedeFisica(reqWorAtt.entity_title, REDE_FISICA_ATUAL)	
...
def topografiaRedeFisica(entityP, redeFisica) :

	if redeFisica == REDE_FISICA_ATUAL : #Primeira versao de rede com apenas 1 switch
...
class SBBWorkspaceManager(object):	
	...
	def workspaceCreate(self):
		...
		if(-1 != registeredWorkspace[0]) :
			...
			#workspaceRegisterDictionary[int(len(workspaceRegisterDictionary)+1)] = self.reqWorAtt.workspace_title
