...
class DTSSocket(socket.socket):
	...
	#def send(self, addr, msg_bufs, msg_bufs1):
		print("Executou")
	...
	def recv_filter(self, addr):
		while True:
			...
			print("Comecando o receiver: ")
			resp = self.recv(1518)	
			print("Acabando o receiver: ", resp)
...
class Entity(object):
	...
	def registera(self):
		...
		msg.title = self.title
		print("inicio do send")
		res = responseSend(self.socket, "DTS\x00\x00\x00", msg, 0)
		print("fim do send")
		if res.status != dts_pb2.ControlResponse.SUCCESS:
	...
	def register(self):
		msg = etcp_pb2.EntityRegister()
		msg.title = self.title
		print("Executando register correto")
