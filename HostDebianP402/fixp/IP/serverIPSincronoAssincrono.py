...
def atualizaListaRecebimento(pktRecebimentoP) :
  ...
  if(int(EXECUTA) == int(EXECUTA_CHAT_RAJADA_INFORMACOES_ASSINCRONO_RECEBIMENTO)) :
    ...
    file = open("RecebimentoPacotesIPAssincronoServer.csv","a")
...
def atualizaListaEnvio(pktEnvioP, dataAtualP, stringPacoteIdP) :
  ...
  listaEnvio.append(["ENVIOC;", #descricao de envio
		     ...
		     stringPacoteCabecalhoId + ";", #dados do pacote para verificacao posterior de taxa de erro
...
def main():
  ...
  if(len(sys.argv) == 4) :
    ...
    if(int(EXECUTA) == int(EXECUTA_CHAT_ENVIO_INFORMACOES_SINCRONA)) :
      print ("  Primeiro parametro (execucao).........: Envio de mensagens forma sincrona - Server")
    elif (int(EXECUTA) == int(EXECUTA_CHAT_RAJADA_INFORMACOES_ASSINCRONO_ENVIO)) :
      print ("  Primeiro parametro (execucao)...........: Envio de mensagens forma assincrona - Envio Server")
    elif (int(EXECUTA) == int(EXECUTA_CHAT_RAJADA_INFORMACOES_ASSINCRONO_RECEBIMENTO)) :
      print ("  Primeiro parametro (execucao)...........: Envio de mensagens forma assincrona - Recebimento Server")
    else :
  ...
  if(int(EXECUTA) == int(EXECUTA_CHAT_ENVIO_INFORMACOES_SINCRONA)) : 
    ...
    gravaStreamingArquivoEnvio("EnvioPacotesIPSincronoServer.csv")
    gravaStreamingArquivoRecebimento("RecebimentoPacotesIPSincronoServer.csv")
  elif (int(EXECUTA) == int(EXECUTA_CHAT_RAJADA_INFORMACOES_ASSINCRONO_ENVIO)) :
    for i in range(int(CONTADOR_ESCUTA_ENVIO)) :
      conteudo = geraConteudo(TAMANHO_PACOTES)        
      primitiveSend(conteudo)
    gravaStreamingArquivoEnvio("EnvioPacotesIPAssincronoServer.csv")
  elif(int(EXECUTA) == int(EXECUTA_CHAT_RAJADA_INFORMACOES_ASSINCRONO_RECEBIMENTO)) : 
    #conteudo = geraConteudo(TAMANHO_PACOTES)    
    #primitiveSend(conteudo)
    filtro = '''ether proto ''' + str(ARQUITETURA_IPV4) + ''' and ip dst ''' + IP_SRC + ''' and
    ...
    #gravaStreamingArquivoRecebimento("RecebimentoPacotesIPAssincronoServer.csv")
