...
class DTSSocket(socket.socket):
	...
	def send(self, addr, switch, port, *msg_bufs):
		...
		byte_seq = itertools.chain((addr, ethertype), switch, port, msg_bufs)
...
class SBBNEConnector(object):
	...
	def sendForwardingRules(self, switch, port, ethernetHeader, reqObj, status):
			...
			for msg_buffer in buffer_splitter(rawData):
				...
				if cont == 0 :
					...
					if reqObj.type == dts_pb2.ControlRequest.ETCP_ENTITY_REGISTER :
						...
						status = worAttCreObj.workspaceCreate()
		...
		for row in switchesPorts :
			...
			if registerIndex == -1 :
				...
				for portsRow in row[1] :
					...
					if portsRow not in forwardingRulesByWorkspace[registerIndex][2][1] :

						forwardingRulesByWorkspace[registerIndex][2][1].append(portsRow)
		...
		#try :
		  ...
		  if(-1 != registeredWorkspace[0]) :
			...
			workspaceRegisterList.append([workspaceId,
						     self.reqWorAtt.workspace_title, 
						     hashlib.sha256(self.reqWorAtt.workspace_title).digest()[:6],
						     workspaceId,
						     registeredEntity[0] ] )
			...
			if(self.reqWorCre.attach_too) :											
				print("Atachando workspace no SBB Entity Manager...")
			        createStatus = self.workspaceAttach()								   		  
